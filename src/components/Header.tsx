  ...
  return (
    <header className={`relative border-b px-4 py-2.5 transition-colors duration-200 ${
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        {/* Title, KMC Logo and Facility Branding */}
        <div className="flex items-center gap-3">
          {/* Official Kiira Motors Corporation (KMC) Logo */}
          <div className={`flex items-center justify-center px-2 py-1 rounded-lg border transition-all ${
            isDark ? 'bg-[#181B22] border-[#2D3139] shadow-inner' : 'bg-white border-slate-200 shadow-sm'
          }`}>
            <KmcLogo height={28} />
          </div>

          <div className={`h-8 w-[1px] ${isDark ? 'bg-[#2D3139]' : 'bg-slate-300'}`} />

          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className={`text-sm font-extrabold tracking-tight uppercase ${isDark ? 'text-white' : 'text-slate-900'}`}>
                Radi Energy Solutions <span className="text-blue-600">Plant Digital Twin</span>
              </h1>
            ...
            <p className={`text-[11px] ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>
              Katuugo Nakasongola Gigafactory • 1,183 Packs/Shift • 26.57s Line Takt • 4-Day Buffer WH
            </p>
          </div>
        </div>

        {/* Live Simulation Ticker & Controls */}
        <div className={`flex flex-wrap items-center gap-3 px-3 py-1 rounded-lg border text-xs font-mono backdrop-blur-sm ${
          ...
          <div className="flex items-center gap-1.5">
            <span className={isDark ? 'text-gray-400' : 'text-slate-500'}>
              <Clock className="w-3.5 h-3.5 inline text-blue-500 mr-1" /> Shift:
            </span>
            <span className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{formatShiftTime(simState.shiftTimeSeconds)}</span>
            <span className="text-[10px] text-gray-400">({Math.max(0, Math.floor(simState.shiftHoursRemaining * 60))}m left)</span>
          </div>

          <div className={`h-3.5 w-[1px] ${isDark ? 'bg-[#2D3139]' : 'bg-slate-300'}`} />

          <div className="flex items-center gap-1.5">
            <span className={isDark ? 'text-gray-400' : 'text-slate-500'}>EV Packs:</span>
            ...
            <span className="text-[10px] text-blue-500 font-semibold">
              ({((simState.goodPacks / Math.max(1, simState.targetPacks)) * 100).toFixed(1)}%)
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <span className={isDark ? 'text-gray-400' : 'text-slate-500'}>BESS Racks:</span>
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className={`p-1 rounded transition-colors ${isDark ? 'text-gray-400 hover:text-white bg-[#0F1115] hover:bg-[#2D3139]' : 'text-slate-600 hover:text-slate-900 bg-white border border-slate-200 hover:bg-slate-100'}`}
        ...
        {/* Right Action Controls: Shift Report, Light/Dark Switcher & AI Strategy */}
        <div className="flex items-center gap-2">
          {/* Shift Report Action Button */}
          <button
            ...
            title="Open Shift Performance & QA Report"
          >
          ...
          >
            {isDark ? (
              <>
                ...
                <span className="text-[11px] text-gray-200">Light Mode</span>
              </>
            ) : (
              <>
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-sm transition-all transform hover:scale-[1.02]"
          >
      ...
      {/* Navigation Tabs */}
      <nav className={`relative z-10 flex flex-wrap items-center gap-1 mt-2.5 pt-2 border-t ${isDark ? 'border-[#2D3139]' : 'border-slate-200'}`}>
        {navItems.map(item => {
