...
import { SimulationState, ProcessZone, WarehouseInfo } from '../types/plant';
...
- Battery Passport QR integration logged at Zone Z8 End-of-Line testing complies with **EU Regulation 2023/1542** for carbon footprint declaration and recycling efficiency.`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#111318] border border-[#2D3139] rounded-xl max-w-2xl w-full p-6 text-[#D1D5DB] shadow-2xl flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
        {/* Modal Header */}
        <div className="flex items-center justify-between border-b border-[#2D3139] pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Generative AI optimization engine for operational efficiency</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1 hover:bg-[#1A1D23] rounded text-gray-400 hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* AI Output Display */}
        {aiReport && (
          <div className="bg-[#090A0D] p-4 rounded-lg border border-blue-500/30 text-xs text-gray-300 font-mono whitespace-pre-line leading-relaxed border-l-4 border-l-blue-500">
            {aiReport}
          </div>
        )}
      </div>
