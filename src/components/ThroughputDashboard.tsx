...
export const ThroughputDashboard: React.FC<ThroughputDashboardProps> = ({
  ...
  const hourlyData = Array.from({ length: 10 }, (_, i) => {
    ...
    // Future hours must be null, not 0 — an area chart plots 0 as a real value,
    // which made the cumulative curve dive back to the axis mid-shift and read
    // as a line stoppage. null ends the series at "now".
    const cumulativeActual: number | null = isCompleted
      ? Math.round(hourlyTarget * hour * (simState.goodPacks / Math.max(1, (simState.shiftTimeSeconds / 3600) * hourlyTarget)))
      : isCurrent
      ? simState.goodPacks
      : null;

    const actualBess = Math.min(bessTarget, Math.floor(((cumulativeActual ?? 0) / Math.max(1, simState.targetPacks)) * bessTarget));

    return {
      hour: `Hr ${hour} (${String(5 + hour).padStart(2, '0')}:00)`,
    ...
    <div className={`flex flex-col gap-6 p-4 md:p-6 pb-16 w-full min-h-full transition-colors duration-200 ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={`p-4 rounded-xl border flex flex-col justify-between transition-all ${
          isDark ? 'bg-[#111318] border-[#2D3139]' : 'bg-[#FDFCFA] border-[#E7E3DC] shadow-sm'
        }`}>
        ...
        <div className={`p-4 rounded-xl border flex flex-col justify-between transition-all ${
          ...
          <div className="my-2 flex items-center justify-between">
            <div>
              <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>Reworked</div>
              <div className="text-xl font-mono text-amber-500 font-semibold">{simState.reworkedPacks}</div>
            </div>
            <div>
              <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>Scrapped</div>
              <div className="text-xl font-mono text-red-500 font-semibold">{simState.scrappedPacks}</div>
            </div>
            <div>
              <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>Passed EOL</div>
          ...
          {zoneLeadTimeModel.map(z => (
            <div
              ...
              className={`p-2.5 rounded-lg border text-xs flex flex-col justify-between ${
                isDark ? 'bg-[#111318] border-[#2D3139]' : 'bg-[#FDFCFA] border-[#E7E3DC]'
              }`}
            >
              <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className={`lg:col-span-7 p-4 rounded-xl border flex flex-col transition-all ${isDark ? 'bg-[#111318] border-[#2D3139]' : 'bg-[#FDFCFA] border-[#E7E3DC] shadow-sm'}`}>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-4 text-xs font-mono">
              ...
              <span className="flex items-center gap-1.5 text-gray-400">
                <span className={`w-2.5 h-2 rounded ${isDark ? 'bg-[#2D3139]' : 'bg-slate-300'}`} /> Hourly Target (118)
              </span>
            </div>
          </div>
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={hourlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="target" name="Target (118)" fill={isDark ? '#2D3139' : '#CBD5E1'} radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className={`lg:col-span-5 p-4 rounded-xl border flex flex-col transition-all ${isDark ? 'bg-[#111318] border-[#2D3139]' : 'bg-[#FDFCFA] border-[#E7E3DC] shadow-sm'}`}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className={`text-sm font-bold uppercase tracking-wider ${isDark ? 'text-white' : 'text-slate-900'}`}>
                Cumulative Shift Progress Curve
              </h3>
              <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>Actual output curve vs target trajectory</p>
            </div>
          </div>
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={hourlyData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                ...
                <CartesianGrid strokeDasharray="3 3" stroke={isDark ? '#2D3139' : '#E2E8F0'} vertical={false} />
                <XAxis dataKey="hour" stroke={isDark ? '#6B7280' : '#64748B'} fontSize={10} tickLine={false} />
                <YAxis stroke={isDark ? '#6B7280' : '#64748B'} fontSize={10} tickLine={false} domain={[0, 1250]} />
                <Tooltip
                  contentStyle={{ backgroundColor: '#1A1D23', borderColor: '#2D3139', color: '#FFF', fontSize: '12px' }}
                />
                <Area type="monotone" connectNulls={false} dataKey="cumulativeActual" name="Actual Cumulative" stroke="#10B981" fillOpacity={1} fill="url(#cumulGrad)" strokeWidth={2} />
      ...
      <div className={`p-4 rounded-xl border transition-all ${isDark ? 'bg-[#111318] border-[#2D3139]' : 'bg-[#FDFCFA] border-[#E7E3DC] shadow-sm'}`}>
